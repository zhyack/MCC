...
class AttentionWrapper(rnn_cell_impl.RNNCell):
  ...
  def call(self, inputs, state):
    ...
    if self.additional_contexts!=None:
      additional_w_1=tf.get_variable('addtional_w_1', shape=[self._attention_layer_size, self._attention_layer_size])
      additional_w_2=tf.get_variable('addtional_w_2', shape=[self._attention_layer_size, self._attention_layer_size])
      additional_b_1=tf.get_variable('addtional_b_1', shape=[self._attention_layer_size])
      additional_b_2=tf.get_variable('addtional_b_2', shape=[self._attention_layer_size])
      additional_output = tf.einsum('ij,jk->ik', self.additional_contexts, additional_w_2)+ additional_b_2
      s = []
      for t in cell_state:
        th = tf.einsum('ij,jk->ik', t.h, additional_w_1) + additional_b_1
        s.append(tf.contrib.rnn.LSTMStateTuple(t.c, th+additional_output))
      cell_state = tuple(s)
    cell_output, next_cell_state = self._cell(cell_inputs, cell_state)

    # if self.additional_contexts!=None:
    #   additional_output = tf.einsum('ij,jk->ik', self.additional_contexts, additional_w_2)+ additional_b_2
    #   cell_output = tf.einsum('ij,jk->ik', cell_output, additional_w_1)+ additional_b_1
