...
import math
from .tools.HAtt import HierarchicalBahdanauAttention, HierarchicalLuongAttention, AttentionWrapper
...
def modelInitHierarchicalAttentionDecoderCell(size, layers, cell_type, input_dropout, output_dropout, encoder_outputs, encoder_outputs_lengths, additional_contexts, max_num_high_layer, high_layer_scores, att_type='LUONG'):
    cells = []
    for _ in range(layers):
        config_cell = None
        if cell_type in ['LSTM', 'lstm']:
    ...
    op_cell = AttentionWrapper(op_cell, attention_mechanism, attention_layer_size=size, additional_contexts=additional_contexts, output_attention=False)
...
def modelRunDecoderForBSInfer(cell, inputs, word_embedding_matrix, beam_width, id_end, max_len, initial_state, output_projection_layer):
    decoder=tf.contrib.seq2seq.BeamSearchDecoder(cell, word_embedding_matrix, inputs, id_end, initial_state=initial_state, beam_width=beam_width, output_layer=output_projection_layer, length_penalty_weight=1.1)
    outputs, states, sequence_lengths = tf.contrib.seq2seq.dynamic_decode(decoder, impute_finished=False, maximum_iterations=max_len*2)
